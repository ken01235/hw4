...
while(True):
    ...
    if (line) :
        ...
        if (abs(diff_dis) <= 30 and diff_dis < 0 and theta > 60) :  # turn left
            uart.write(("/turn/run 100 0.8 \n").encode())
            print("right")
        elif(abs(diff_dis) <= 30 and diff_dis > 0 and theta > 60) : # turn right
            uart.write(("/turn/run 100 -0.6 \n").encode())
            print("left")
